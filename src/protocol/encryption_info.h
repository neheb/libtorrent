...
public:
  ...
  void                set_encrypt(std::unique_ptr<RC4> encrypt)   { m_encrypt = std::move(encrypt); m_encrypted = m_obfuscated = true; }
  void                set_decrypt(std::unique_ptr<RC4> decrypt)   { m_decrypt = std::move(decrypt); m_decryptValid = true; }
...
private:
  ...
  std::unique_ptr<RC4> m_encrypt;
  std::unique_ptr<RC4> m_decrypt;
};
