...
PeerConnectionBase::initialize(DownloadMain* download, PeerInfo* peerInfo, SocketFd fd, Bitfield* bitfield, EncryptionInfo* encryptionInfo, ProtocolExtension* extensions) {
  ...
  m_peerChunks.upload_throttle()->slot_activate() = [ObjectPtr = static_cast<SocketBase*>(this)] { ObjectPtr->receive_throttle_up_activate(); };

  m_peerChunks.download_throttle()->set_list_iterator(m_down->throttle()->end());
  m_peerChunks.download_throttle()->slot_activate() = [ObjectPtr = static_cast<SocketBase*>(this)] { ObjectPtr->receive_throttle_down_activate(); };
