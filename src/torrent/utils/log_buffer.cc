...
log_open_log_buffer(const char* name) {
  ...
  auto buffer = log_buffer_ptr(new log_buffer, [](auto lb) { log_buffer_deleter(lb); });

  log_open_output(name, [&buffer](auto data, auto len, auto group) { buffer->lock_and_push_log(data, len, group); });
  return buffer;
}
