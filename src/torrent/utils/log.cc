...
log_group::internal_print(const HashString* hash, const char* subsystem, const void* dump_data, size_t dump_size, const char* fmt, ...) {
  ...
  std::for_each(m_first, m_last, [this, begin = static_cast<const char*>(first), data = static_cast<const char*>(buffer)](auto log) {
    log(data, std::distance(data, begin), std::distance(log_groups.begin(), this));
  });
  if (dump_data != NULL) {
    std::for_each(m_first, m_last, [dump_data, dump_size](auto log) { log(static_cast<const char*>(dump_data), dump_size, -1); });
  }
