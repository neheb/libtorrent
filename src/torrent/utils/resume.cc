...
resume_load_progress(Download download, const Object& object) {
  ...
  for (auto listItr = fileList->begin(), listLast = fileList->end(); listItr != listLast; ++listItr, ++filesItr) {
    ...
    if (mtimeValue == ~static_cast<int64_t>(0) || mtimeValue == ~static_cast<int64_t>(1)) {
      ...
      if (mtimeValue == ~static_cast<int64_t>(0)) {
    ...
    if (mtimeValue == ~static_cast<int64_t>(3)) {
    ...
    if (mtimeValue == ~static_cast<int64_t>(2) || mtimeValue != fs.modified_time()) {
...
resume_save_progress(Download download, Object& object) {
  ...
  if (!download.is_hash_checked()) {
    ...
    for (auto& file : files)
      file.insert_key("mtime", ~static_cast<int64_t>(2));
  ...
  for (auto listItr = fileList->begin(), listLast = fileList->end(); listItr != listLast; ++listItr, ++filesItr) {
    ...
    if (!fileExists) {
      ...
      if ((*listItr)->is_create_queued()) {
        ...
        filesItr->insert_key("mtime", ~static_cast<int64_t>(0));
        LT_LOG_SAVE_FILE("file not created, create queued", 0);
      } else {
        // ~1 means the file shouldn't be created.
        filesItr->insert_key("mtime", ~static_cast<int64_t>(1));
      ...
      filesItr->insert_key("mtime", ~static_cast<int64_t>(3));
...
resume_save_tracker_settings(Download download, Object& object) {
  ...
  for (auto tracker : *tracker_list) {
    ...
    if (tracker.is_extra_tracker()) {
      trackerObject.insert_key("extra_tracker", Object(static_cast<int64_t>(1)));
