...
DownloadWrapper::DownloadWrapper() :
  ...
  m_main->delay_download_done().slot()       = [capture0 = data()] { capture0->call_download_done(); };
  m_main->delay_partially_done().slot()      = [capture0 = data()] { capture0->call_partially_done(); };
  m_main->delay_partially_restarted().slot() = [capture0 = data()] { capture0->call_partially_restarted(); };
...
DownloadWrapper::initialize(const std::string& hash, const std::string& id) {
  ...
  info()->slot_left()      = [capture0 = m_main->file_list()] { return capture0->left_bytes(); };
  info()->slot_completed() = [capture0 = m_main->file_list()] { return capture0->completed_bytes(); };
